...
module.exports.run = async (bot, message, args) => {
    ...
    if(!args[0]) {
        ...
        .addField(`Commands:`, "``doggo`` ``warn`` ``pay`` ``poll`` ``serverinfo`` ``userinfo`` ``prefix`` ``coins`` ``meme`` ``clear`` ``say`` ``kick`` ``ban`` ``mute`` ``tempmute`` ``status`` ``8ball`` ``report`` ``addrole`` ``removerole`` ``level`` ``coins`` ``pay`` ``botinfo`` ``avatar`` ``roles``")
